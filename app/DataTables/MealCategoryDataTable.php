...
class MealCategoryDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->addColumn('action', 'dashboard.page.meal_category.btn.btn')
            ->addIndexColumn()
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('name')->data('name.ar')->title('اسـم التصنيف (بالعربية)	'),
            Column::make('name')->data('name.en')->title('اسـم التصنيف (بالإنجليزيـة)	'),
